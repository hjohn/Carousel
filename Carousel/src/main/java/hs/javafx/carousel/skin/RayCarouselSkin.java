...
public class RayCarouselSkin<T> extends AbstractCarouselSkin<T> {
  ...
  public class RayLayoutPass extends AbstractLayoutPass<RayLayoutItem> {
    ...
    public RayLayoutPass(double fractionalIndex) {
      ...
      double fractionalCellCount = getSkinnable().getWidth() / getMaxCellWidth() * getRadiusRatio() * getCarouselViewFraction() * getDensity() * Math.PI * 2;
      this.fractionalCellCount = fractionalCellCount < 3 ? 3 : fractionalCellCount;
